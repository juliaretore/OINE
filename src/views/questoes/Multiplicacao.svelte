<script>
    ...
    onMount(async () => {
        ...
        tentativas = await carregarTentativas(3, usuarioLogado, questao);
        todasAsTentativas = await carregarTentativasPorUsuarioTipo(3, usuarioLogado);
    ...
    async function conferir() {
        resultados.forEach(r => {
            const resultadoNum = r.num1 * r.num2;;
        ...
        if(acertos === 3) {
            await salvarTentativa(acertos, 3, questao, usuarioLogado);
            tentativas = await carregarTentativas(3, usuarioLogado, questao);
            todasAsTentativas = await carregarTentativasPorUsuarioTipo(3, usuarioLogado);
            concluido = true;
    ...
    $: if (concluido) {
        resultados.forEach(r => {
            r.respNum = simplificarFracao(r.num1 * r.num2, r.den1 * r.den2).num;
            r.respDen = simplificarFracao(r.num1 * r.num2, r.den1 * r.den2).den;
            r.correto = true;
        });
    }
</script>
...
<header class="app-header">
    ...
    <div class="nav-options">
        ...
        <a href="/multiplicacao" use:link class="active">Multiplicação</a>
